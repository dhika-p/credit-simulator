...
docker run -it --rm <username-dockerhub>/credit-simulator

# mode file, memakai file_inputs.txt yang sudah ada di dalam image
docker run --rm <username-dockerhub>/credit-simulator file_inputs.txt

# mode file, memakai file sendiri
docker run --rm -v "$(pwd)/data.txt:/app/data.txt" <username-dockerhub>/credit-simulator data.txt
```
...
docker run -it --rm -e CREDIT_SIMULATOR_API_URL="https://contoh.com/loan.json" <username-dockerhub>/credit-simulator
```
...
- Mulai tahun ke-2, bunga naik **0,1%** di tahun genap dan **0,5%**
### Rumus cicilan
...
Workflow ada di [`.github/workflows/ci-master.yml`]
